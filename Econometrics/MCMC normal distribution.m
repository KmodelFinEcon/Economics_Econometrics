...
function MCMC_LinearRegress()
    ...
    for i = 2:Q
        ...
        for j = 1:4
            ...
            est_current = MH(proposal, j, est_current, x1, x2, y);
        end
    ...
    for i = 1:4
        ...
        set(h,'edgecolor', [0.4, 0.4, 1]);
        title(sprintf('%s (\\mu = %.3f, \\sigma = %.3f)', ...
            titles{i}, mcmc_mean(i), mcmc_std(i)), 'FontSize', 10);
    end
end
...
function params = MH(proposal, index, params, x1, x2, y)
    params_new = params;
